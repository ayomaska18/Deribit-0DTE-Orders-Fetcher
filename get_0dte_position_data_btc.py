...
def filter_orders(trades: list, instrument_name: str):
    ...
    save_trades_to_csv(buy_calls, 'BTC_buy_calls')
    save_trades_to_csv(sell_calls, 'BTC_sell_calls')
    save_trades_to_csv(buy_puts, 'BTC_buy_puts')
    save_trades_to_csv(sell_puts, 'BTC_sell_puts')
...
def plot_vol_graph():
    buy_calls_df = pd.read_csv('Orders/BTC-5AUG24_buy_calls.csv', header=None, names=['timestamp', 'trade_id', 'instrument_name', 'direction', 'price', 'amount', 'strike'])
    sell_calls_df = pd.read_csv('Orders/BTC-5AUG24_sell_calls.csv', header=None, names=['timestamp', 'trade_id', 'instrument_name', 'direction', 'price', 'amount', 'strike'])
    buy_puts_df = pd.read_csv('Orders/BTC-5AUG24_buy_puts.csv', header=None, names=['timestamp', 'trade_id', 'instrument_name', 'direction', 'price', 'amount', 'strike'])
    sell_puts_df = pd.read_csv('Orders/BTC-5AUG24_sell_puts.csv', header=None, names=['timestamp', 'trade_id', 'instrument_name', 'direction', 'price', 'amount', 'strike'])
...
def main():
    ...
    instruments_BTC = get_instruments('BTC', 'option', 'true')
    
    BTC_0dte = get_0dte_instruments(instruments_BTC, start_timestamp, end_timestamp)

    for instrument in BTC_0dte:
